...
A heat pump uses work input to transfer heat from a cold reservoir ($T_c$) to a higher-temperature space ($T_h$):
...
Where:
- $Q_h$ ($P_h$) = heat output (or thermal power) delivered to hot sink
...
The theoretical upper limit for a reversible heat pump operating between $T_h$ and $T_c$:

$$
\mathrm{COP}_{\max} = \frac{T_h}{T_h - T_c}
$$

*(Temperatures $T_h$ and $T_c$ must be in **Kelvin**).*
